...
listenClick('.social-links', function () {
    $('.social-links-add').
        append('        <div class="col-lg-6 mb-7 social-links-div">\n' +
            '                            <div class="d-flex">\n' +
            ...
            defaultProfileUrl + ') ;width: 40px; height: 40px"></div>\n' +
...
listenClick('.social_link_save', function (e) {
    ...
    for (var i = 0; i < inputs.length; i++) {
        if ($.trim($(inputs[i]).val()) == '') {
            displayErrorMessage(
                Lang.get('messages.flash.social_links_is_required'))
            return false
        }
    }
    for (var i = 0; i < img.length; i++) {
        ...
        if (image == '') {
            displayErrorMessage(
                Lang.get('messages.flash.social_links_img_is_required'))
            return false
        }
