    ...
    <div class="nav-tabs nav flex-column nav-pills mb-7 mt-3" id="v-pills-tab" role="tablist"
        ...
        <div class="row justify-content-center mt-3">
            ...
            <a href="{{ route('add-contact', $vcard->id) }}"
                class="btn btn-primary fs-14">{{ __('messages.setting.add_contact') }}</a>
            </div>
            @endif
            <div class="col-12 text-center">
                <a onclick="downloadVcard('{{ $vcard->name }}.vcf',{{ $vcard->id }})" class="btn btn-primary fs-14">
                    <i class="fas fa-download download-icon"></i>
                    <div class="spinner-border fs-1 d-none" role="status" style="width: 25px; height: 25px;"><span class="visually-hidden">Loading...</span></div>
                        {{ __('messages.vcard.download_vcard') }}
                </a>
            </div>
        </div>
</header>
