...
class VcardController extends AppBaseController
{
    ...
    public function addContact(Vcard $vcard)
    {
        ...
        $vcfVcard->addPhoneNumber($vcard->phone);
        $vcfVcard->addPhoneNumber($vcard->alternative_phone);
        $vcfVcard->addAddress($vcard->location);
        $socialLinks = SocialLink::whereVcardId($vcard->id)->first();
        $websiteLink  = $socialLinks->website ??  $vcard->url_alias;
        $vcfVcard->addURL($websiteLink);
        $vcfVcard->addPhoto($vcard->getMedia(\App\Models\Vcard::PROFILE_PATH)->first()->getPath());
        return $vcfVcard->download();
    }
}
